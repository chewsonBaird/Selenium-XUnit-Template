...
internal static class Initializer
{
    ...
    internal static void Run() {
        ...
        string[] attributes = { "Xunit.FactAttribute", "Xunit.SkippableFactAttribute", "Xunit.TheoryAttribute", "Xunit.SkippableTheoryAttribute" };
...
class GoToUrlPatch
{
    ...
    static Exception Finalizer(Exception __exception)
    {
        ...
        if (driver != null)
        {
            var sessionName = Regex.Replace((new StackTrace()).GetFrame(1).GetMethod().ToString().Split(" ")[1].Split("(")[0], @"(_Patch)(\d+)(?!.*\d)", "");
var status = "passed";var message = "";
            ...
            if(__exception != null)
{return new BrowserStackException(message, Regex.Replace(__exception.StackTrace, @"(_Patch)(\d+)", ""));
}
